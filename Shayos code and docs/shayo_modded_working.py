...
# Gemmi suggeted better file listing code tHU 26/2/2026 ph

    def list_files(self, directory: str = "/") -> List[Dict]:
        try:
        # FlashAir prefers directory paths without a trailing slash for op=100
            clean_dir = directory.rstrip('/') if directory != "/" else "/"
            response = requests.get(
                f"{self.ip}/command.cgi", 
                params={'op': 100, 'DIR': clean_dir}, 
                timeout=10
            )
            response.raise_for_status()
            
            # Split lines and remove the 'WLANSD_FILELIST' header
            lines = response.text.strip().split('\r\n')
            if not lines or "WLANSD_FILELIST" not in lines[0]:
                return []

            files = []
            for line in lines[1:]:
                parts = line.split(',')
                # Standard format: [path_or_empty, filename, size, attribute, date, time]
                if len(parts) >= 6:
                    try:
                        files.append({
                            'filename': parts[1],
                            'size': int(parts[2]),
                            'is_directory': bool(int(parts[3]) & 0x10),
                            'date': int(parts[4]),
                            'time': int(parts[5])
                        })
            ...
            print(f"FlashAir Error: {e}") # Log the actual error for debugging
            return []
# ENDS GEMINI CODE
